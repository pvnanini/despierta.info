...
class Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		if ( empty( $input ) ) {
			return new WP_Error('invalid_input', __( 'Error Occurred', 'creative-addons-for-elementor' ) );
		}
		...
		foreach ($input as $key => $input_value) {
			...
			$defaults = array(
				'label'       => __( 'Label', 'creative-addons-for-elementor' ),
				'type'        => self::TEXT,
				'mandatory'   => true,
				'max'         => '20',
				'min'         => '3',
			...
			if ( is_wp_error($result) ) {
                ...
                Logging::add_log( 'Please change the value of ' . $field_spec['label'] . ' field. Current value: "' . $input_value . '" - ' . $result->get_error_message() . ', code: ' . $result->get_error_message(), $result );
	...
	private function filter_input_field( $value, $field_spec ) {
		...
		switch ( $field_spec['type'] ) {
			...
			default:
				return new WP_Error('eckb-invalid-input-type', __( 'Error Occurred', 'creative-addons-for-elementor' ) . ' - ' . $field_spec['type']);
		}
	}
	...
	private function filter_select( $value, $field_spec ) {
		...
		if ( ! in_array( $value, array_keys($field_spec['options']) )  && ! empty($field_spec['mandatory']) ) {
			$value_text = ( empty($value) ? __( 'empty', 'creative-addons-for-elementor' ) : '"' . $value . '"' );
			$msg = '<br>' . sprintf( __( 'The value cannot be %s. %sValid values are: %s', 'creative-addons-for-elementor' ), $value_text, '<br>', implode(", ", $field_spec['options']) );
	...
	private function filter_checkbox( $value ) {
		...
		return new WP_Error('filter_checkbox_invalid', sprintf( __( 'The value "%s" is not valid', 'creative-addons-for-elementor' ), $value ) );
	}
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		$number_int = Utilities::sanitize_int( $number, null );
		if ( $number != $number_int ) {
			return new WP_Error('filter_not_number',sprintf( __( 'The value "%s" is not a number', 'creative-addons-for-elementor' ), $number ) );
		}

		if ( $number > $field_spec['max'] ) {
			$msg = sprintf( __( 'The value %s is larger than maximum of %s', 'creative-addons-for-elementor' ), $number, $field_spec['max'] );
			return new WP_Error( 'filter_not_number', $msg );
		} else if ( $number < $field_spec['min'] ) {
			$msg = sprintf( __( 'The value %s is smaller than minimum of %s', 'creative-addons-for-elementor' ), $number, $field_spec['min'] );
			return new WP_Error( 'filter_not_number', $msg );
		}
	...
	private function filter_true_false( $boolean ) {
		...
		return new WP_Error( 'filter_not_number', __( 'Error Occurred', 'creative-addons-for-elementor' ) . ' (112)' );
	}
	...
	public function filter_color_hex( $value, $field_spec=array() ) {
		...
		return new WP_Error('filter_not_color_hex', sprintf( __( 'The value "%s" is not valid HEX color. Expected format: #xxxxxx', 'creative-addons-for-elementor' ), $value ));
	}
	...
	private function filter_id( $id ) {
		$id = Utilities::sanitize_get_id( $id );
		if ( is_wp_error($id) ) {
			return new WP_Error('filter_not_id', __( 'Error Occurred', 'creative-addons-for-elementor' ) . ' - ' . $id . ' - ' . $id->get_error_code() );
		}
		return $id;
	}
	...
	private function filter_enumeration( $value, $field_spec ) {
		...
		return new WP_Error('filter_not_enumeration', __( 'Error Occurred', 'creative-addons-for-elementor' ) . ' - ' . $value );
	}
