...
class Countries
{
	...
	public function get_allowed_countries() {
		if ('all' === get_option('rtcl_allowed_countries')) {
			return apply_filters('rtcl_countries_allowed_countries', $this->countries);
		}

		if ('all_except' === get_option('rtcl_allowed_countries')) {
			$except_countries = get_option('rtcl_all_except_countries', []);
	...
	public function get_default_address_fields() {
		...
		if ('optional' === get_option('rtcl_checkout_address_2_field', 'optional')) {
		...
		$fields = [
			...
			'company'    => [
				...
				'priority'     => 30,
				'required'     => 'required' === get_option('rtcl_checkout_company_field', 'optional'),
			],
			'country'    => [
				'type'         => 'country',
			...
			'address_2'  => [
				...
				'priority'     => 60,
				'required'     => 'required' === get_option('rtcl_checkout_address_2_field', 'optional'),
			],
			'city'       => [
		...
		if ('hidden' === get_option('rtcl_checkout_company_field', 'optional')) {
			unset($fields['company']);
		}

		if ('hidden' === get_option('rtcl_checkout_address_2_field', 'optional')) {
	...
	public function get_address_fields($country = '', $type = 'billing_') {
		...
		if ('billing_' === $type) {
			if ('hidden' !== get_option('rtcl_checkout_phone_field', 'required')) {
			...
			$address_fields['billing_email'] = [
				...
				'class'        => ['form-row-wide'],
				'validate'     => ['email'],
				'autocomplete' => 'no' === get_option('rtcl_registration_generate_username') ? 'email' : 'email username',
				'priority'     => 110,
			];
		}
		...
		// Sort each of the fields based on priority.
		uasort($address_fields, 'rtcl_checkout_fields_uasort_comparison');
