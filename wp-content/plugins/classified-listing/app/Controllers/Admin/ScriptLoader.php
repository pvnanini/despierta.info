...
class ScriptLoader {
	...
	function register_script_both_end() {
		...
		if ( Functions::has_map() ) {
			...
			if ( 'google' === $map_type && $map_api_key = Functions::get_option_item( 'rtcl_misc_settings', 'map_api_key' ) ) {
				...
				wp_register_script( 'rtcl-map', rtcl()->get_assets_uri( "js/gmap.js" ), array(
					'jquery',
					'rtcl-google-map'
				), $this->version, true );
			} else if ( 'osm' === $map_type ) {
				wp_register_script( 'rtcl-map', rtcl()->get_assets_uri( "js/osm-map.js" ), array( 'jquery' ), $this->version, true );
			}
	...
	function register_admin_script() {
		...
		wp_register_style( 'rtcl-admin', rtcl()->get_assets_uri( "css/rtcl-admin.min.css" ), array( 'rtcl-bootstrap' ), $this->version );
