...
class EPKB_Upgrades {
	...
	public function __construct() {
		...
		add_action( 'activated_plugin', array( 'EPKB_Upgrades', 'initial_addons_setup' ), 20, 2 );
	...
	/**
	 * Display license screen on addon first activation or upgrade
	 *
	 * @param $plugin
	 * @param $network_activation
	 */
	public static function initial_addons_setup( $plugin, $network_activation ) {

		// return if activating from network or doing bulk activation
		if ( $network_activation || isset( $_GET['activate-multi'] ) ) {
			return;
		}

		// return if not echo addon
		if ( ! preg_match( '/echo-/', $plugin ) ) {
			return;
		}
		...
		foreach ( $addons as $addon ) {
			...
			// check is addon enabled
			if ( ! EPKB_Utilities::is_plugin_enabled( $addon ) ) {
				continue;
			}

			// check is addon not recently activated
			$plugin_installed = get_transient( "_{$addon}_plugin_activated" );
