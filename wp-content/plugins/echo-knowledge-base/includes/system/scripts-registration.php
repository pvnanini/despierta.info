...
function epkb_add_admin_bar_button( WP_Admin_Bar $wp_admin_bar ) {
	...
	if ( ! empty( $title ) ) {
		...
		$url = EPKB_Core_Utilities::is_kb_flag( 'editor_backend_mode' ) ?
			admin_url( '/edit.php?post_type=' . EPKB_KB_Handler::get_post_type( EPKB_KB_Handler::get_current_kb_id() ) . '&page=epkb-kb-configuration#kb-design' ) :
...
function epkb_load_editor_styles() {
	...
	$epkb_editor_params = array(
		...
		'config_backend_mode_link'  	=> admin_url( '/edit.php?post_type=' . EPKB_KB_Handler::get_post_type( EPKB_KB_Handler::get_current_kb_id() ) . '&page=epkb-kb-configuration&action=enable_editor_backend_mode&_wpnonce_epkb_ajax_action=' . wp_create_nonce( '_wpnonce_epkb_ajax_action' ) . '#kb-design' ),
