        ...
        if ( empty( $already_deleted ) ) {  ?>
		    <form id="epkb-delete-all-data__form" class="epkb-delete-all-data__form" method="POST">
                <p class="epkb-delete-all-data__form-title">    <?php
	                echo sprintf( esc_html__( 'Write "%s" in the below input box if you want to delete ALL KB content immediately. You cannot undo this action.', 'echo-knowledge-base' ), 'delete' ); ?>
                </p>    <?php
                EPKB_HTML_Elements::text_basic( array(
				    'value' => '',
				    'name'    => 'epkb_delete_text',
			    ) );
                EPKB_HTML_Elements::submit_button_v2( __( 'Delete All', 'echo-knowledge-base' ), 'epkb_delete_all_kb_data', '', '', false, '', 'epkb-error-btn' );  ?>
            </form> <?php
