...
class EPKB_Settings_Controller {
	...
	public function toggle_debug() {
		...
		if ( empty( $is_debug_on ) ) {
			set_transient( EPKB_Settings_Controller::EPKB_DEBUG, true, DAY_IN_SECONDS );
		} else {
			delete_transient( EPKB_Settings_Controller::EPKB_DEBUG );
		}
	...
	public function show_logs() {
		...
		set_transient( EPKB_Settings_Controller::EPKB_SHOW_LOGS, true, HOUR_IN_SECONDS );
	...
	public function enable_advanced_search_debug() {
		...
		if ( empty( $is_debug_on ) ) {
			set_transient( EPKB_Settings_Controller::EPKB_ADVANCED_SEARCH_DEBUG, true, DAY_IN_SECONDS );
		} else {
			delete_transient( EPKB_Settings_Controller::EPKB_ADVANCED_SEARCH_DEBUG );
		}
	...
	public function download_debug_info() {
		...
		EPKB_Utilities::save_wp_option( EPKB_Settings_Controller::EPKB_DEBUG, false );
		...
		$output = '';
		if ( $debug_box == 'main' ) {
			$output = EPKB_Configuration_Tools_Page::display_debug_data();
		}
		if ( $debug_box == 'asea' ) {
			$output = EPKB_Configuration_Tools_Page::display_asea_debug_data();
		}
