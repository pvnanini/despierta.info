...
class EPKB_Configuration_Page {
	...
	private function show_frontend_editor_links() {
        ...
        echo '<div class="epkb-editor-launcher-mode-title">' . esc_html__( 'Editor Launcher Mode' , 'echo-knowledge-base' ) . '</div>';

		EPKB_HTML_Elements::radio_buttons_horizontal( array(
			'name'                 => 'editor_backend_mode',
			'input_group_class' => 'epkb-radio-horizontal-button-group-container',
	...
	private function show_multilingual_settings() {
		...
		EPKB_HTML_Elements::checkbox_toggle( array(
							...
							'text'          => 'WPML and Polylang Enable',
							'textLoc'       => 'left',
							'topDesc'       => '<a href="https://www.echoknowledgebase.com/documentation/translate-text/" target="_blank">' . esc_html__( 'Follow WPML setup instructions here.', 'echo-knowledge-base' ) . '</a>',
							'checked'       => ( ! empty( $this->kb_config['wpml_is_enabled'] ) && $this->kb_config['wpml_is_enabled'] == 'on' ),
						) );
		echo '<input type="hidden" id="_wpnonce_epkb_ajax_action" name="_wpnonce_epkb_ajax_action" value="' . wp_create_nonce( "_wpnonce_epkb_ajax_action" ) . '"/>';
	...
	private function get_kb_location_box() {
		...
		// If at least one KB Main Page exists for the current KB
			...
			// If user has multiple pages with KB Shortcode then let them know but this is normal for WPML users
	...
	private function display_kb_url_help_box() {
		...
		return EPKB_HTML_Forms::notification_box_middle( array(
			...
			'desc'  => sprintf( '%s <a href="%s" class="ret" target="_blank">%s</a>', esc_html__( 'Need to change KB URLs?', 'echo-knowledge-base' ), 'https://www.echoknowledgebase.com/documentation/changing-permalinks-urls-and-slugs/', esc_html__( 'Learn More', 'echo-knowledge-base' ) )
		), true  );
	}
		...
		<div class="epkb-admin__section-wrap epkb-admin__deprecated-wizard-warning"><?php
			...
			EPKB_HTML_Forms::notification_box_popup( array(
				...
				'desc'  => '<span>' . esc_html__( 'You have an old version of articles format. Please run the upgrade to continue. After the upgrade, check your articles and make minor adjustments if required.', 'echo-knowledge-base' ) . ' ' .
				 '<a href="' . esc_url( $editor_url ) . '" class="epkb-primary-btn">' . esc_html__( 'UPGRADE NOW', 'echo-knowledge-base' ) . '</a></span>' // TODO button
